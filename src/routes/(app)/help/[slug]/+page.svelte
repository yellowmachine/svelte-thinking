...
<div class="mx-auto max-w-3xl px-6 py-10">
	...
	<!-- Breadcrumb -->
	<nav class="mb-6 flex items-center gap-2 font-sans text-xs text-ink-faint dark:text-dark-ink-faint">
		<a href="/help" class="text-accent hover:underline underline-offset-2">Help</a>
		<span>/</span>
	...
	<div class="mb-8">
		...
		<p class="mt-3 font-sans text-sm leading-relaxed text-ink-muted dark:text-dark-ink-muted max-w-2xl">
			{sample.description}
		</p>
	</div>
	...
	<!-- What you can do -->
	<section class="mb-8">
		<h2 class="mb-3 font-serif text-base font-semibold text-ink dark:text-dark-ink">What you can do</h2>
		<ul class="space-y-1.5">
			{#each sample.features as feature}
				<li class="flex items-start gap-2 font-sans text-sm text-ink-muted dark:text-dark-ink-muted">
	...
	<section class="mb-8">
		<h2 class="mb-3 font-serif text-base font-semibold text-ink dark:text-dark-ink">
			Example — <span class="font-sans font-normal text-ink-muted dark:text-dark-ink-muted">{exampleDoc.title}</span>
		</h2>
		<div class="overflow-hidden rounded-xl border border-paper-border dark:border-dark-paper-border">
			<div class="flex items-center justify-between border-b border-paper-border bg-paper-ui px-4 py-2 dark:border-dark-paper-border dark:bg-dark-paper-ui">
				<span class="font-mono text-[11px] text-ink-faint dark:text-dark-ink-faint">markdown</span>
				<span class="rounded bg-paper-border px-2 py-0.5 font-mono text-[10px] text-ink-muted dark:bg-dark-paper-border dark:text-dark-ink-muted">
					type: {exampleDoc.docType}
				</span>
			</div>
			<pre class="overflow-x-auto px-5 py-4 font-mono text-xs leading-relaxed text-ink dark:text-dark-ink bg-paper dark:bg-dark-paper whitespace-pre-wrap">{exampleDoc.content.trim()}</pre>
		</div>

		{#if sample.docs.length > 1}
			<p class="mt-2 font-sans text-xs text-ink-faint dark:text-dark-ink-faint">
				This sample project includes {sample.docs.length} documents. The example above shows the first one.
			</p>
		{/if}
	</section>

	<!-- Try it -->
	<section class="rounded-xl border border-accent/20 bg-accent/5 px-6 py-5 dark:border-accent/15 dark:bg-accent/10">
		...
		<p class="mt-1 mb-4 font-sans text-sm text-ink-muted dark:text-dark-ink-muted">
			...
			pre-filled with {sample.docs.length === 1 ? 'this example document' : `${sample.docs.length} example documents`}.
