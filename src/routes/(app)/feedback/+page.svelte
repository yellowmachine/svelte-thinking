...
<div class="mx-auto max-w-2xl px-6 py-10">
	<div class="mb-8">
		<h1 class="font-serif text-2xl font-semibold text-ink dark:text-dark-ink">Community suggestions</h1>
		<p class="mt-1 font-sans text-sm text-ink-muted dark:text-dark-ink-muted">
			{data.entries.length} {data.entries.length === 1 ? 'suggestion' : 'suggestions'} from beta users.
		</p>
	</div>

	{#if data.entries.length === 0}
		<div class="rounded-2xl border border-dashed border-paper-border py-16 text-center dark:border-dark-paper-border">
			<p class="font-sans text-sm text-ink-faint dark:text-dark-ink-faint">No suggestions yet. Be the first.</p>
		</div>
	{:else}
