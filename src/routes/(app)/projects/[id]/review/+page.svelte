	...
	{#if data.documents.length === 0}
		...
		>
			<svg
				...
				viewBox="0 0 24 24"
				fill="none"
				class="mb-3 text-ink-faint dark:text-dark-ink-faint"
			>
				<path
			...
			{#each data.documents as doc (doc.documentId)}
				<section id="doc-{doc.documentId}">
					...
					<div class="mb-3 flex items-center gap-2">
						...
						<span
							class="rounded-full bg-amber-100 px-2 py-0.5 font-sans text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
						>
							{doc.threads.length}
						</span>
					</div>
