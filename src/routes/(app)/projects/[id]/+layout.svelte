<script lang="ts">
	...
	$effect(() => {
		workspaceStore.syncToOrg(data.projectOrgId);
	});
...
{#if scheduledDeleteAt}
	<div
		class="border-b border-red-200 bg-red-50 px-4 py-3 dark:border-red-900/40 dark:bg-red-950/30"
	>
		<div class="mx-auto flex max-w-5xl items-center justify-between gap-4">
			<div class="flex items-center gap-2.5">
				...
				<p class="font-sans text-sm text-red-800 dark:text-red-300">
					...
					<strong
						>{daysLeft(scheduledDeleteAt)} day{daysLeft(scheduledDeleteAt) === 1 ? '' : 's'}</strong
					>
					({scheduledDeleteAt.toLocaleDateString('en', { day: 'numeric', month: 'long' })}). Export
