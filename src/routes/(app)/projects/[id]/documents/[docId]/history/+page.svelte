<script lang="ts">
	...
	let diffData = $state<{ current: { versionNumber: number; content: string }; previous: { versionNumber: number; content: string } | null } | null>(null);
	...
	async function restoreVersion(v: Version) {
		if (!confirm(`¿Restaurar v${v.versionNumber} como borrador? El draft actual se perderá.`)) return;
		restoring = true;
		try {
...
<div class="flex h-screen flex-col bg-paper dark:bg-dark-paper">
	<!-- Header -->
	<header class="sticky top-0 z-10 flex shrink-0 items-center gap-2 border-b border-paper-border bg-paper/95 px-6 py-3 backdrop-blur-sm dark:border-dark-paper-border dark:bg-dark-paper/95">
		<a
		...
		<span class="font-sans text-xs text-ink-faint dark:text-dark-ink-faint">
			{data.versions.length} {data.versions.length === 1 ? 'versión' : 'versiones'}
		</span>

		{#if restoredVersion !== null}
			<span class="ml-auto rounded-md border border-green-300 bg-green-50 px-2.5 py-1 font-sans text-xs text-green-700 dark:border-green-700/50 dark:bg-green-900/20 dark:text-green-400">
				v{restoredVersion} restaurada como borrador
			</span>
		{/if}
	</header>
	...
	<div class="flex flex-1 overflow-hidden">
		<!-- Version list -->
		<aside class="flex w-72 shrink-0 flex-col overflow-hidden border-r border-paper-border bg-paper dark:border-dark-paper-border dark:bg-dark-paper">
			{#if data.versions.length === 0}
				<div class="px-6 py-12 text-center font-sans text-sm text-ink-muted dark:text-dark-ink-muted">
					No hay versiones publicadas aún.
				</div>
			{:else}
				<ul class="flex-1 divide-y divide-paper-border overflow-y-auto dark:divide-dark-paper-border">
					{#each data.versions as v (v.id)}
						<li class="px-4 py-3 {selectedVersion?.id === v.id ? 'bg-paper-ui dark:bg-dark-paper-ui' : ''}">
							...
							<div class="mt-2 flex items-center gap-1.5">
								<button
									...
									class="rounded px-2 py-1 font-sans text-xs transition-colors {selectedVersion?.id === v.id && panelMode === 'preview' ? 'bg-accent text-white' : 'text-ink-muted hover:bg-paper-ui dark:text-dark-ink-muted dark:hover:bg-dark-paper-ui'}"
								>
									Ver
								</button>
								<button
									onclick={() => selectVersion(v, 'diff')}
									class="rounded px-2 py-1 font-sans text-xs transition-colors {selectedVersion?.id === v.id && panelMode === 'diff' ? 'bg-accent text-white' : 'text-ink-muted hover:bg-paper-ui dark:text-dark-ink-muted dark:hover:bg-dark-paper-ui'}"
								>
									Diff
								</button>
								{#if data.canWrite}
			...
			{#if selectedVersion === null}
				<div class="flex h-full items-center justify-center">
					<p class="font-sans text-sm text-ink-faint dark:text-dark-ink-faint">
						Selecciona una versión para ver su contenido o el diff.
					</p>
				</div>
			{:else if loading}
				<div class="flex h-full items-center justify-center">
			...
			{:else if panelMode === 'preview' && versionContent !== null}
				<div class="mx-auto max-w-2xl px-6 py-10">
					<p class="mb-6 font-sans text-xs text-ink-faint dark:text-dark-ink-faint">
						v{selectedVersion.versionNumber} · {selectedVersion.changeDescription ?? 'Sin descripción'}
					</p>
