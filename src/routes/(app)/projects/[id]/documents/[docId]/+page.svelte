<script lang="ts">
	...
	async function runReviewSelection(reviewType: ReviewType) {
		...
		selectionReview = { from, to, coords: coords ? { bottom: coords.bottom, left: coords.left } : { bottom: 0, left: 0 }, reviewType, loading: true, suggestion: '', explanation: '' };
		try {
			...
			selectionReview = { ...selectionReview!, loading: false, suggestion: res.suggestion, explanation: res.explanation };
		} catch (e) {
...
<div class="flex overflow-hidden" style="height: calc(100vh - 57px)">
	...
	<div class="relative flex-1 overflow-y-auto px-6 py-10">
			...
			>
				...
				{#if hasAiKey && currentSelection && currentSelection.text.trim().length > 20}
					<div class="pointer-events-auto relative">
						<button
							class="rounded-md bg-paper border border-paper-border px-3 py-1.5 font-sans text-xs font-semibold text-ink shadow-md transition-colors hover:bg-paper-muted dark:bg-dark-paper dark:border-dark-paper-border dark:text-dark-ink dark:hover:bg-dark-paper-muted"
							onclick={() => (showReviewTypeMenu = !showReviewTypeMenu)}
						>
							Review ▾
						</button>
						{#if showReviewTypeMenu}
							<div class="absolute top-full left-0 mt-1 z-30 w-44 rounded-md border border-paper-border bg-paper shadow-lg dark:border-dark-paper-border dark:bg-dark-paper">
								{#each Object.entries(reviewTypeLabels) as [type, label]}
									<button
										class="block w-full px-3 py-2 text-left font-sans text-xs text-ink hover:bg-paper-muted dark:text-dark-ink dark:hover:bg-dark-paper-muted"
		...
		{#if selectionReview && selectionReview.coords}
			...
			>
				<div class="pointer-events-auto w-96 rounded-lg border border-paper-border bg-paper shadow-lg dark:border-dark-paper-border dark:bg-dark-paper">
					<div class="flex items-center justify-between border-b border-paper-border px-3 py-2 dark:border-dark-paper-border">
						<span class="font-sans text-xs font-semibold text-ink dark:text-dark-ink">{reviewTypeLabels[selectionReview.reviewType]}</span>
						<button
					...
					<div class="px-3 py-2.5 space-y-2">
						...
						{:else}
							<p class="font-sans text-xs leading-relaxed text-ink dark:text-dark-ink whitespace-pre-wrap">{selectionReview.suggestion}</p>
							<p class="font-sans text-[10px] text-ink-faint dark:text-dark-ink-faint italic">{selectionReview.explanation}</p>
							...
							<p class="font-sans text-[10px] text-ink-faint dark:text-dark-ink-faint">Generated by AI · may be inaccurate</p>
						{/if}
					</div>
				</div>
			</div>
