...
export const load: LayoutServerLoad = async (event) => {
  ...
  const rows = await event.locals.withRLS((db) =>
    db
      .select({ orgId: project.orgId, ownerId: project.ownerId, scheduledDeleteAt: project.scheduledDeleteAt })
      .from(project)
  ...
  if (scheduledDeleteAt && scheduledDeleteAt <= new Date()) {
    ...
    const s3 = await resolveProjectS3Config(projectId, userId, event.locals.withRLS).catch(() => null);
    if (s3) {
