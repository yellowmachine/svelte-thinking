...
import { parseTaskConfig, type AiTask, type TaskConfig } from '$lib/ai-config';
import { getValidOpenRouterModelIds } from '$lib/server/openrouter';
...
export const load: LayoutServerLoad = async (event) => {
	...
	const [
		activeNotifications,
		[pending, profile, ownedOrgs, memberOrgs, aiKeys, userS3ConfigKey],
		validAiModelIds
	] = await Promise.all([
		db
			.select({
			...
			Promise.all([
				...
				tx
					.select({
						...
						aiTaskConfig: userProfile.aiTaskConfig,
	...
	const aiTaskConfig = parseTaskConfig(profile[0]?.aiTaskConfig ?? null);
	const obsoleteAiTasks = validAiModelIds
		? (Object.entries(aiTaskConfig) as [AiTask, TaskConfig][])
				.filter(([, cfg]) => !validAiModelIds.has(cfg.model))
				.map(([task, cfg]) => ({ task, model: cfg.model }))
		: [];
	...
	return {
		...
		orgs,
		aiTaskConfig,
		completedTutorials: (profile[0]?.completedTutorials ?? []) as string[],
