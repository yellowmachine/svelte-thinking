<script lang="ts">
	...
	const isExpired = $derived(new Date(data.invitation.expiresAt) < new Date());
	const isAlreadyAccepted = $derived(data.invitation.status === 'accepted');
	...
	<div class="w-full max-w-md">
		<div class="rounded-2xl border border-paper-border bg-paper p-8 dark:border-dark-paper-border dark:bg-dark-paper">
			...
			{:else if isExpired || data.invitation.status === 'rejected'}
				<div class="text-center">
					<div class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-paper-border dark:bg-dark-paper-border">
						<svg width="24" height="24" viewBox="0 0 24 24" fill="none">
							<circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.5" class="text-ink-faint dark:text-dark-ink-faint" />
							<path d="M12 8v4M12 16h.01" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" class="text-ink-faint dark:text-dark-ink-faint" />
						</svg>
					</div>
					<h1 class="font-serif text-2xl font-semibold text-ink dark:text-dark-ink">
						Invitation expired
			...
			{:else}
				<div class="mb-1 font-sans text-xs font-medium uppercase tracking-widest text-ink-faint dark:text-dark-ink-faint">
				...
				<div class="mt-5 flex items-center gap-3 rounded-lg bg-paper-ui px-4 py-3 dark:bg-dark-paper-ui">
					<div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-accent font-sans text-xs font-semibold text-white">
						{data.invitation.invitedEmail[0].toUpperCase()}
					</div>
					<div>
						<p class="font-sans text-sm text-ink dark:text-dark-ink">{data.invitation.invitedEmail}</p>
						<p class="font-sans text-xs text-ink-faint dark:text-dark-ink-faint">Invited as <span class="font-medium text-accent">member</span></p>
					</div>
				</div>

				{#if reqState === 'error'}
					<p class="mt-4 rounded-lg bg-red-50 px-4 py-3 font-sans text-sm text-red-700">{errorMsg}</p>
				{/if}
