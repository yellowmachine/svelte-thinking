...
export const POST: RequestHandler = async (event) => {
  ...
  const { projectId, documentTitle, documentContent, spellLanguage, message, conversationId: inputConvId, modelOverride } = parsed.data;
  ...
  const [
    ...
    resolveTaskKey(withRLS, rawDb, user.id, 'grammar', projectId, 'anthropic/claude-haiku-4-5').catch(() =>
      resolveTaskKey(withRLS, rawDb, user.id, 'agent', projectId)
    )
  ]);
  ...
  (async () => {
    try {
      ...
      send({
        ...
        conversationId: savedConvId,
        messageId: msgId,
        pendingEditorActions: result.pendingEditorActions.length > 0 ? result.pendingEditorActions : undefined,
