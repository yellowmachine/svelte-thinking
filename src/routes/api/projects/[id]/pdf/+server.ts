...
export const GET: RequestHandler = async (event) => {
	...
	const [proj, requirements, rawRefs, collaborators] = await Promise.all([
		event.locals.withRLS((db) =>
		...
		) as Promise<
			{
				...
				requirementsTemplate: string | null;
				doi: string | null;
				version: string | null;
		...
		event.locals
			...
			) as Promise<(typeof reference.$inferSelect)[]>,
	...
	if (fulfilledReqs.length === 0) {
		error(422, 'The project has no completed requirements for export');
	}
	...
	if (sections.length === 0) {
		error(422, 'The assigned documents don’t have any content yet.');
	}
