...
{#if open}
	<div class="fixed inset-0 z-50 flex items-end justify-end p-6 sm:items-center sm:justify-center">
		<!-- Backdrop -->
		<button class="absolute inset-0 bg-black/30 backdrop-blur-sm" onclick={close} aria-label="Close"
		></button>
			...
			>
				<div>
					...
					<p class="mt-0.5 font-sans text-xs text-ink-faint dark:text-dark-ink-faint">
						We're in beta — your feedback shapes Scholio.
					</p>
				</div>
				<button
					onclick={close}
			...
			<div class="p-5">
				{#if done}
					<div class="flex flex-col items-center gap-2 py-6 text-center">
						...
						<p class="font-serif text-base font-medium text-ink dark:text-dark-ink">Thank you!</p>
				...
				{:else}
					<textarea
						bind:value={message}
						rows="4"
						placeholder="What would you improve? What's missing? Anything not working well?"
					...
					<div
						class="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-900/40 dark:bg-amber-950/20"
					>
					...
					<label class="mt-3 flex cursor-pointer items-center gap-2.5">
						...
						<span class="font-sans text-sm text-ink-muted dark:text-dark-ink-muted"
							>Show my name alongside the feedback</span
						>
					</label>
