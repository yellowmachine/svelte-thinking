<script lang="ts">
	...
	const originalText = $derived(action.type === 'replace_text' ? action.anchorText : action.anchorText);
...
{#if status !== 'discarded'}
	...
	>
			...
			{#if status === 'idle'}
				...
				<div class="mt-2.5 space-y-1.5">
					<div class="rounded-lg bg-red-50 px-2.5 py-2 dark:bg-red-950/30">
						<p class="mb-0.5 font-sans text-[10px] font-semibold uppercase tracking-wide text-red-500 dark:text-red-400">Before</p>
						<p class="font-mono text-xs leading-relaxed text-red-700 line-through dark:text-red-300">
							{truncate(originalText)}
						</p>
					</div>
					<div class="rounded-lg bg-green-50 px-2.5 py-2 dark:bg-green-950/30">
						<p class="mb-0.5 font-sans text-[10px] font-semibold uppercase tracking-wide text-green-600 dark:text-green-400">After</p>
		...
		<!-- Actions -->
		<div class="flex justify-end gap-2 px-3 pb-2.5 pt-2">
			{#if status === 'done'}
				<span class="font-sans text-xs text-green-600 dark:text-green-400">✓ Applied to document</span>
			{:else}
				<button
					type="button"
					onclick={discard}
