		...
		try {
			...
			urlRefResults = all
				.filter((r) =>
					...
					(r.authors as { first: string; last: string }[]).some((a) => a.last.toLowerCase().includes(lower))
				)
				.slice(0, 8)
...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" role="dialog" aria-modal="true">
	...
	<div class="absolute inset-0" onclick={() => { reset(); onclose(); }}></div>
	<div class="relative w-full max-w-lg rounded-2xl border border-paper-border bg-paper p-6 shadow-2xl dark:border-dark-paper-border dark:bg-dark-paper">
		...
		{#if urlStep === 'input' || urlStep === 'extracting'}
			...
			<input
				...
				autofocus
				onkeydown={(e) => { if (e.key === 'Enter' && hasAiKey) runUrlAiExtract(); }}
			...
			<div class="mt-4 flex justify-end gap-2">
				<button onclick={() => { reset(); onclose(); }} class="rounded-md border border-paper-border px-4 py-2 font-sans text-sm text-ink-muted hover:bg-paper-ui dark:border-dark-paper-border dark:text-dark-ink-muted">
					Cancelar
				</button>
				{#if hasAiKey}
					<button
