<script lang="ts">
	...
	const fulfilledDocTitle = $derived(
		requirement.fulfilledDocumentId
			? (documents.find((d) => d.id === requirement.fulfilledDocumentId)?.title ??
					'(deleted document)')
			: ''
	);
...
>
	<div class="flex items-start gap-3 p-4">
		...
		<div class="mt-0.5 shrink-0">
			...
			{:else}
				<div
					class="h-5 w-5 rounded-full border-2 border-paper-border dark:border-dark-paper-border"
				></div>
			{/if}
		</div>
		...
		<div class="min-w-0 flex-1">
			...
			<div class="flex flex-wrap items-center gap-2">
				<span class="font-sans text-sm font-semibold text-ink dark:text-dark-ink"
					>{requirement.name}</span
				>
				{#if !requirement.required}
					<span
						class="rounded-full bg-paper-border px-2 py-0.5 font-sans text-[10px] font-medium tracking-wide text-ink-faint uppercase dark:bg-dark-paper-border dark:text-dark-ink-faint"
					>
						Optional
					</span>
				{/if}
			...
			{:else if isOwner}
				{#if pickerOpen}
					<div
						class="mt-2 rounded-lg border border-paper-border bg-paper-ui p-2 dark:border-dark-paper-border dark:bg-dark-paper-ui"
					>
		...
		{#if isOwner}
			<button
				...
				class="shrink-0 rounded p-1 text-ink-faint opacity-0 transition-opacity group-hover:opacity-100 hover:text-ink dark:text-dark-ink-faint dark:hover:text-dark-ink"
			>
				<svg
					width="13"
					height="13"
