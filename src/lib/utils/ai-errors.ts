...
export function classifyAiError(e: unknown): { kind: AiErrorClass; message: string } {
	...
	if (code === 'PRECONDITION_FAILED') {
		return {
			kind: 'system',
			message: 'No API key configured. Go to Settings → AI Assistant to add one.'
		};
	}
	...
	if (/crédit|credit|402/i.test(msg)) {
		return {
			kind: 'system',
			message: 'Insufficient credits on OpenRouter. Top up your account to continue.'
		};
	}
	if (/inválid|invalid.*key|401/i.test(msg)) {
		return {
			kind: 'system',
			message: 'Invalid API key. Check your key in Settings → AI Assistant.'
		};
	}
	if (/límite|rate.?limit|429/i.test(msg)) {
		return {
			kind: 'system',
			message: 'Rate limit reached on OpenRouter. Wait a moment before trying again.'
		};
	}
