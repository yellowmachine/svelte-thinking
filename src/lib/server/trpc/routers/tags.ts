...
export const tagsRouter = router({
		...
		.mutation(async ({ ctx, input }) => {
			...
			const rows = await ctx.withRLS((db) =>
				db
					.select()
					.from(tag)
					.where(and(eq(tag.userId, ctx.user.id), eq(tag.name, input.name)))
					.limit(1)
			);
	...
	delete: protectedProcedure
		.input(z.string())
		.mutation(({ ctx, input: tagId }) =>
			ctx.withRLS((db) => db.delete(tag).where(and(eq(tag.id, tagId), eq(tag.userId, ctx.user.id))))
		),

	addToProject: protectedProcedure
		.input(z.object({ projectId: z.string(), tagId: z.string() }))
		.mutation(({ ctx, input }) =>
			ctx.withRLS((db) => db.insert(projectTag).values(input).onConflictDoNothing())
		),
	...
	removeFromProject: protectedProcedure
		...
		.mutation(({ ctx, input }) =>
			ctx.withRLS((db) =>
				db
					.delete(projectTag)
					.where(and(eq(projectTag.projectId, input.projectId), eq(projectTag.tagId, input.tagId)))
			)
		)
});
