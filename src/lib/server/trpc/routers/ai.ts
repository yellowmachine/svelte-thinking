...
async function throwProviderError(res: Response): Promise<never> {
	...
	try {
		...
		if (code === 401)
			message = `API key de ${providerLabel} inválida. Revísala en Ajustes → Asistente IA.`;
		else if (code === 402)
			message = `Has agotado los créditos de ${providerLabel}. Recarga tu cuenta.`;
		else if (code === 429)
			message = `Límite de peticiones alcanzado en ${providerLabel}. Espera un momento.`;
...
export async function buildProjectIndex(withRLS: WithRLS, projectId: string): Promise<string> {
	...
	const [proj, docs, reqs, refCount, theologyCount, projectIssues] = await Promise.all([
		withRLS((db) =>
			...
			{ title: string; description: string | null; requirementsTemplate: string | null }[]
		>,
		...
		withRLS((db) =>
			db
				.select({
					...
					content: sql<
						string | null
					>`CASE WHEN length(${documentVersion.content}) < ${SUMMARY_MIN_CHARS} THEN ${documentVersion.content} ELSE NULL END`,
	...
	if (docs.length > 0) {
		...
		for (const doc of docs) {
			...
			if (doc.aiSummary) {
				...
				lines.push(
					`*(~${Math.round(doc.contentLength / 6).toLocaleString()} words — summary pending. Use read_document to read full content.)*`
				);
			} else {
	...
	if (reqs.length > 0) {
		...
		lines.push(
			`REQUIREMENTS (${fulfilled}/${reqs.length} completed, use get_requirement_details for descriptions):`
		);
	...
	if (projectIssues.length > 0) {
		...
		lines.push(
			`ISSUES (${open.length} open / ${projectIssues.length} total, use list_issues for full details):`
		);
		...
		function: {
			...
			parameters: {
				...
				properties: {
					...
					content: {
						type: 'string',
						description: 'Issue description or body in Markdown (optional)'
					},
					priority: {
						type: 'string',
						...
						items: {
							...
							properties: {
								citeKey: {
									type: 'string',
									description:
										'BibTeX cite key (e.g. bueno1985religion). Auto-generated if omitted.'
								},
								type: {
									type: 'string',
									enum: [
...
async function executeTool(
	...
	switch (name) {
		case 'read_document': {
			...
			if (!rows[0])
				return { output: `Error: document "${id}" not found in this project.`, docsUsed: [] };
		...
		case 'search_documents_semantic': {
			...
			if (!rows.length)
				return {
					output: `No results for "${query}". Documents may not be indexed yet.`,
					docsUsed: []
				};
		...
		case 'list_references': {
			...
			if (refs.length === 0)
				return { output: 'This project has no bibliography entries yet.', docsUsed: [] };
		...
		case 'get_requirement_details': {
			...
			if (reqs.length === 0)
				return { output: 'This project has no requirements defined yet.', docsUsed: [] };
		...
		case 'list_issues': {
			...
			return {
				output: issues
					.map((iss) => {
						const icon = STATUS_ICON[iss.status] ?? '○';
						const preview = iss.content
							? ` — ${iss.content.slice(0, 120).replace(/\n/g, ' ')}${iss.content.length > 120 ? '…' : ''}`
							: '';
...
export async function resolveTaskKey(
	...
	if (orgId) {
		...
		if (memberRows[0] || ownerRows[0]) {
			...
			const orgKeyRow = (
				orgTaskEntry ? orgKeys.find((k) => k.id === orgTaskEntry.keyId) : orgKeys[0]
			) as KeyRow | undefined;
			...
			if (orgKeyRow) {
				...
				throw new TRPCError({
					...
					message:
						'Configure organization OpenRouter API key in Settings → Organization → AI Assistant.'
				});
			}
		}
	}
...
async function runAgentLoop(
	...
	for (let i = 0; i < MAX_AGENT_ITERATIONS; i++) {
		...
		if (!choice)
			throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'No response from model.' });
				...
				choice.message.tool_calls.map(async (tc) => {
					...
					if (tc.function.name === 'create_document') {
						pendingActions.push({
							...
							docType:
								(args.type as Extract<PendingAction, { type: 'create_document' }>['docType']) ||
								'paper',
					...
					if (tc.function.name === 'create_issue') {
						pendingActions.push({
							...
							priority:
								(args.priority as Extract<PendingAction, { type: 'create_issue' }>['priority']) ||
								'medium'
						});
						return {
							role: 'tool' as const,
					...
					if (tc.function.name === 'propose_references') {
						const refs = ((args.references as ProposedReference[]) || []).map(
							(r) =>
								({
									citeKey: r.citeKey,
									type: r.type || 'misc',
					...
					const { output, docsUsed } = await executeTool(
						tc.function.name,
						args,
						withRLS,
						projectId
		...
		function: {
			...
			parameters: {
				...
				properties: {
					anchorText: {
						type: 'string',
						description: 'Exact substring to replace (verbatim copy from document)'
					},
		...
		function: {
			...
			parameters: {
				...
				properties: {
					anchorText: {
						type: 'string',
						description: 'Exact substring after which to insert (verbatim copy)'
					},
...
async function runEditorAgentLoop(
	...
	content: string;
	pendingEditorActions: PendingEditorAction[];
	pendingActions: PendingAction[];
	inputTokens: number;
	outputTokens: number;
}> {
				...
				choice.message.tool_calls.map(async (tc) => {
					...
					if (tc.function.name === 'replace_text') {
						...
						return {
							role: 'tool' as const,
							tool_call_id: tc.id,
							content: 'Replacement proposed. The user will see a preview.'
						};
					}
					...
					if (tc.function.name === 'list_references') {
						...
						if (rows.length === 0) {
							return {
								role: 'tool' as const,
								tool_call_id: tc.id,
								content: 'This project has no bibliography entries yet.'
							};
						}
						const output = rows
							.map(({ ref: r }) => {
...
${documentContent}`;
							const spellRes = await fetch(OPENROUTER_URL, {
								method: 'POST',
								headers: {
									Authorization: `Bearer ${toolCtx.spellApiKey}`,
									'Content-Type': 'application/json',
									...extraHeaders
								},
								body: JSON.stringify({
									model: toolCtx.spellModel,
							...
							if (spellRes.ok) {
								...
								const raw = JSON.parse(
									(spellData.choices[0]?.message?.content ?? '[]')
										.trim()
										.replace(/^```(?:json)?\n?/, '')
										.replace(/\n?```$/, '')
								...
								const corrections = Array.isArray(raw)
									? raw.filter(
											(c) => c.original && c.suggestion && documentContent.includes(c.original)
										)
									: [];
								for (const c of corrections) {
									pendingEditorActions.push({
										type: 'replace_text',
										anchorText: c.original,
										replacement: c.suggestion,
								...
								return {
									...
									content:
										corrections.length > 0
											? `Spell check complete. ${corrections.length} correction(s) proposed.`
						...
						return {
							role: 'tool' as const,
							tool_call_id: tc.id,
							content: 'Spell check failed. Ask the user to use the Spell button instead.'
						};
					}
...
${documentContent}`;
							const grammarRes = await fetch(OPENROUTER_URL, {
								method: 'POST',
								headers: {
									Authorization: `Bearer ${toolCtx.grammarApiKey}`,
									'Content-Type': 'application/json',
									...extraHeaders
								},
								body: JSON.stringify({
									model: toolCtx.grammarModel,
									messages: [{ role: 'user', content: prompt }],
									temperature: 0,
									max_tokens: 2000
								})
							...
							if (grammarRes.ok) {
								const grammarData = (await grammarRes.json()) as {
									choices: { message: { content: string } }[];
								};
								const raw = JSON.parse(
									(grammarData.choices[0]?.message?.content ?? '[]')
										.trim()
										.replace(/^```(?:json)?\n?/, '')
										.replace(/\n?```$/, '')
								) as { original: string; suggestion: string; explanation: string }[];
								...
								return {
									...
									content:
										corrections.length > 0
											? `Grammar check complete. ${corrections.length} correction(s) proposed.`
											: 'Grammar check complete. No issues found.'
								};
							}
						} catch {
							/* fall through */
						...
						return {
							role: 'tool' as const,
							tool_call_id: tc.id,
							content: 'Grammar check failed. Ask the user to use the Grammar button instead.'
						};
					}
					...
					if (tc.function.name === 'find_untagged') {
						try {
							const enrichRows = (await toolCtx
								.withRLS((db) =>
									(db as Db).execute(
										sql`SELECT * FROM scholio.find_untagged_in_document(${toolCtx.documentId}, ${toolCtx.projectId})`
									)
								)
							...
							if (!enrichRows) {
								return {
									...
									tool_call_id: tc.id,
									content:
										'Person/reference tagging is not available from the chat yet. Ask the user to use the Enrich button in the toolbar.'
								};
							}
						} catch {
							/* fall through */
				...
				toolCalls.map(async (tc) => {
					...
					if (tc.function.name === 'create_document') {
						pendingActions.push({
							...
							title: (args.title as string) || 'Nuevo documento',
							docType:
								(args.type as string as Extract<
									PendingAction,
									{ type: 'create_document' }
								>['docType']) || 'paper',
							content: (args.content as string) || '',
					...
					if (tc.function.name === 'create_issue') {
						pendingActions.push({
							...
							content: (args.content as string) || undefined,
							priority:
								(args.priority as string as Extract<
									PendingAction,
									{ type: 'create_issue' }
								>['priority']) || 'medium'
						});
	...
	getConversation: protectedProcedure
		...
		.query(async ({ ctx, input: conversationId }) => {
			const [conv, messages] = await Promise.all([
				ctx.withRLS((db) =>
					db.select().from(aiConversation).where(eq(aiConversation.id, conversationId)).limit(1)
	...
	sendMessage: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!convId) {
				...
				await ctx.withRLS((db) =>
					db
						...
						.values({ id: convId!, projectId: input.projectId, userId, title })
				);
			} else {
			...
			const effectiveModel =
				!resolvedOrgId && input.modelOverride ? input.modelOverride : resolvedModel;
	...
	applyAction: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			if (input.action.type === 'create_document') {
				...
				ctx
					.withRLS((db) => indexDocument(db, docId, input.projectId, action.content))
			...
			const existingKeys = (await ctx.withRLS((db) =>
				(db as Db)
					.select({ citeKey: reference.citeKey })
					.from(reference)
			...
			for (const refData of action.references) {
				...
				if (!baseCiteKey) {
					const firstAuthorLast =
						refData.authors[0]?.last?.toLowerCase().replace(/[^a-z]/g, '') || 'anon';
...
- Use the language of the document.`;
			...
			try {
				...
				parsed = {
					requirements: Array.isArray(p.requirements)
						...
						: [],
					uncitedRefs: Array.isArray(p.uncitedRefs)
						? p.uncitedRefs.filter((r: unknown) => typeof r === 'string')
						: []
				};
			} catch {
	...
	generateReviewQuestions: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const docContent = docRows[0].draftContent?.trim() ?? '';
			if (!docContent)
				throw new TRPCError({ code: 'BAD_REQUEST', message: 'The document is empty.' });
...
- Use the language of the document.`;
			...
			try {
				...
				if (Array.isArray(parsed))
					questions = parsed.filter((q): q is string => typeof q === 'string').slice(0, 5);
			} catch {
				questions = [];
			}
			...
			try {
				...
				return {
					persons: Array.isArray(parsed.persons)
						? parsed.persons.filter((p): p is string => typeof p === 'string').slice(0, 10)
						: [],
					refs: Array.isArray(parsed.refs)
						? parsed.refs
								.filter(
									(r): r is { context: string; text: string; citeKey: string } =>
										typeof r?.context === 'string' &&
										typeof r?.text === 'string' &&
										typeof r?.citeKey === 'string'
								)
								.slice(0, 10)
						: []
				};
	...
	reviewSelection: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const typeInstructions: Record<string, string> = {
				...
				argument:
					'Evaluate logical strength. Identify unsupported claims, missing premises, or weak reasoning.',
				citations:
					'Identify claims that need bibliographic support. Note which statements are unsourced.',
				terminology:
					'Check terminological consistency. Flag ambiguous or shifting use of key terms.'
			};
